...
- **Rendu des annotations Style/Technique (`rendu.py`, durci au jalon FA3)** : ne JAMAIS borner les marques sur les runs Word entiers — les points de découpe doivent obligatoirement fusionner les bornes de runs ET les bornes de chaque annotation pour produire des segments ATOMIQUES exacts (`_segments` collecte {0, len, bornes Formes, bornes marques}, trie, émet chaque bloc Forme UNE seule fois et les segments texte multi-marqués avec classes cumulées). Les entrees reçues par `preparer_document` sont déjà en coordonnées du TEXTE COURANT (projetées) — les bornes sont directement comparables aux runs projetés.
- **Toggle masquage de l'atelier (durci au jalon FA3)** : `preparer_document()` ne doit JAMAIS retirer les paragraphes sans correction de la liste envoyée au client ; tous les paragraphes projetés sont transmis avec leur état (`nb_masques` = compteur informatif) et le toggle filtre EN MÉMOIRE côté client.
- **Compteur de réévaluation (`atelier.py`, durci au jalon FA2)** : ne JAMAIS régénérer des identifiants `c-r0001…` avec un compteur local réinitialisé à chaque appel ; l'unicité des IDs est continue à l'échelle du document (`_prochain_numero_reevaluation` — max des `c-XXXX`/`c-rXXXX` existants). Par ailleurs : la réévaluation purge les choix des corrections retirées (pas de refus fantôme), les conflits Forme sont résolus DYNAMIQUEMENT à chaque choix (une refusée ne bloque pas), l'obsolescence après modification manuelle touche TOUTES les phases, et les phases de réévaluation s'exécutent en parallèle (`asyncio.gather`).
...
- **Format de `corrections.data_json` (durci au jalon FA3)** : accepter TOUJOURS les deux formats au chargement (`charger_corrections`) — dict par phase (R1-b, `{"forme": [...], ...}`) ET liste plate legacy d'avant R1-b (`[{...}, ...]`) ; un `.values()` naïf sur une liste levait `AttributeError` → Erreur 500 à l'ouverture de l'atelier. 
