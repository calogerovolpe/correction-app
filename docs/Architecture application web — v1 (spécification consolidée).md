...
43. **Segmentation atomique, document complet et atelier résilient (jalon FA3, arbitré le 2026-09-09)** : (a) `rendu._segments` découpe chaque paragraphe aux **points de découpe ATOMIQUES** (fusion des bornes du paragraphe, de chaque Forme appliquée et de chaque marque Style/Technique/Forme refusée) — **fin du sur-marquage au run Word entier** : une annotation ne déborde jamais sur le texte voisin ; les classes se CUMULENT sur un segment couvert par plusieurs marques (un groupe de clic parmi les couvrants, les autres restent accessibles dans la barre latérale) ; un bloc Forme n'est émis qu'une fois (les intervalles internes à sa zone sont absorbés par le bloc del/ins entier) ; (b) `preparer_document` expose **TOUS les paragraphes** du texte (fin du filtrage prématuré backend qui neutralisait le toggle — décision 35 appliquée au contrat de rendu), `nb_masques` conservé pour informer l'UI ; (c) `atelier.charger_corrections` accepte **les deux formats** de `corrections.data_json` (dict par phase R1-b ET liste plate legacy d'avant R1-b) — fin de l'Erreur 500 à l'ouverture des analyses anciennes ; (d) `Atelier.svelte` recharge l'état sur **toute variation d'`analyseId`** (`$effect` — fin de l'atelier figé sur « Chargement… ») et, en cas d'échec de chargement, affiche un bandeau avec **« Réessayer de charger l'atelier »** et « Revenir à l'accueil » — plus d'impasse utilisateur.
44. **Choix de l'IA de correction à la soumission (jalon FA6, arbitré le 2026-09-10)** : l'auteur choisit le modèle texte Mistral qui corrigera SON texte **AU MOMENT de la soumission (E3)**, parmi un catalogue figé dans `app/llm/catalogue.py` (`mistral-small-latest` « Recommandé », `mistral-large-latest` « Haute précision », `open-mistral-nemo` « Rapide », `ministral-8b-latest` « Compact`) ; le modèle choisi s'applique à TOUTES les phases de CETTE analyse ; `modele` absent ou hors catalogue → la configuration `.env` par phase (`APP_MODELE_*`) reste maîtresse (**repli transparent, jamais de blocage**) ; le choix est mémorisé côté serveur (`dernieres_options`) et côté navigateur (`localStorage`) — re-proposé à la soumission suivante. UI E3 : cartes radio stylisées (design tokens, WCAG AA, badge « Recommandé », description pédagogique).
45. **Cohérence transactionnelle de l'atelier (jalon FA6, arbitré le 2026-09-10)** : l'état `documents` porte un **compteur de révision** (`revision`, initial 1) incrémenté à CHAQUE `sauver_etat` ; le contrat `GET /api/v1/analyses/{id}/atelier` l'expose et le frontend Svelte le retransmet avec **chaque mutation** (`choix-forme`, `editer`, `appliquer-alternative`, `appliquer-embellissement`, `reevaluer` — query `revision`). Une révision périmée est refusée **AVANT toute modification** (`ErreurAtelier` statut 409 : « L'atelier a été modifié dans un autre onglet… Rechargez la page ») — plus aucun écrasement silencieux d'état concurrent ; `revision` absent (routes Jinja2, appels non révisés) → aucune vérification (compatibilité conservée). L'atelier Svelte resynchronise automatiquement l'affichage après un 409 (rechargement + bandeau explicite).
