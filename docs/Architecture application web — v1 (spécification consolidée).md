...
* **Onglets hybrides (jalon R1-a)** : la barre d'onglets `Tout | Forme | Style | Technique` (+ `Embellissement` conditionnel) remplace les pastilles-filtres cumulables. L'onglet actif circule via la query string (GET `?onglet=`) et un champ de formulaire (POST) — après une action (choix Forme, réévaluation, alternative, embellissement), on reste sur l'onglet ; onglet inconnu → « Tout ». La projection par phase (`preparer_document_par_phase`) filtre les corrections puis réutilise `preparer_document` : aucune logique de segments dupliquée, **zéro appel LLM en plus** (les 3 passes parallèles sont inchangées).
